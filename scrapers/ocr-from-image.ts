...
export async function scrapeOcrFromImage(
  ...
  try {
    ...
    if (!contentType || !contentType.startsWith("image/")) {
      console.error(
        `[OCR] ERRO: A URL ${imagemURL} não retornou uma imagem. Retornou Content-Type: ${contentType}.`
      );
      return [];
    }
    imageBuffer = Buffer.from(response.data);
    console.log(
      `[OCR] Imagem baixada com sucesso (${(imageBuffer.length / 1024).toFixed(
        2
      )} KB).`
    );
  } catch (error) {
    ...
    const reason = axios.isAxiosError(error)
      ? `${error.response?.status || error.code || "sem status"} ${
          error.message
        }`
      : String(error);
    console.error(
      `[OCR] ERRO: Falha ao baixar a imagem de ${imagemURL}: ${reason}`
    );
    return [];
  }
  ...
  try {
    ...
    try {
      await worker.setParameters({
        tessedit_char_whitelist:
          "0123456789:hH.- abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZáàãâéêíóôõúçÁÀÃÂÉÊÍÓÔÕÚÇ",
      });
    ...
    if (todosHorarios.length === 0) {
      ...
      console.log(
        `[OCR] Processamento finalizado. ${todosHorarios.length} horários encontrados e categorizados.`
      );
    }
