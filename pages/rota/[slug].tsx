...
type Props = {
  origem: string;
  destino: string;
  rotas: Rota[];         // ida: origem → destino
  rotasVolta: Rota[];    // volta: destino → origem
  slug: string;
};
...
export const getStaticProps: GetStaticProps<Props> = async ({ params }) => {
  ...
  // Busca todas as rotas e tenta fazer match pelo slug
  ...
  // Encontra a combinação origem/destino que gera este slug
  let origem = "";
  let destino = "";
  ...
  const rotas = todasRotas.filter(
    (r) => r.origem === origem && r.destino === destino
  );

  const rotasVolta = todasRotas.filter(
...
// ── Componente de tabela de horários ─────────────────────────────────────
function TabelaHorarios({
  ...
  const horariosDoDia = rota.horarios.filter(
    (h) => h.diaDaSemana === diaAtivo && h.sentido === "ida"
  );
        ...
        <tfoot>
          <tr className="border-t bg-muted/20">
            <td colSpan={3} className="px-4 py-2 text-xs text-muted-foreground">
              ...
              {" · "}
              <span className={`inline rounded-full px-2 py-0.5 text-xs font-medium border ${corBadge}`}>
                {rota.empresa.nome}
              </span>
              {rota.tarifaComum && (
                <span className="ml-2">· {formatTarifa(rota.tarifaComum)}</span>
              )}
            </td>
          </tr>
        </tfoot>
...
// ── Card de empresa com comparativo ──────────────────────────────────────
function CardEmpresa({ rota, diaAtivo }: { rota: Rota; diaAtivo: string }) {
  const slug = rota.empresa.slug;
  const corBadge = `${empresaCor[slug] ?? "bg-muted text-muted-foreground border-border"} ${empresaCorDark[slug] ?? ""}`;
  const totalHoje = rota.horarios.filter(
    (h) => h.diaDaSemana === diaAtivo && h.sentido === "ida"
  ).length;
    ...
    <div className="rounded-xl border bg-card overflow-hidden">
      <div className="flex items-center justify-between gap-3 px-4 py-3 border-b bg-muted/20">
        <div className="flex items-center gap-2">
          ...
          <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold border ${corBadge}`}>
            {rota.empresa.nome}
          </span>
          {rota.linha && (
          ...
          <div className="text-right">
            {rota.tarifaComum && (
              <p className="text-sm font-semibold text-foreground">
                {formatTarifa(rota.tarifaComum)}
              </p>
            )}
            {rota.tarifaEstudante && (
              <p className="text-xs text-green-600 dark:text-green-400">
                Estudante {formatTarifa(rota.tarifaEstudante)}
              </p>
            )}
          </div>
          {rota.empresa.sourceUrl && (
            <a
              href={rota.empresa.sourceUrl}
              target="_blank"
              rel="noreferrer"
...
// ── Página principal ──────────────────────────────────────────────────────
export default function RotaPage({ origem, destino, rotas, rotasVolta, slug }: Props) {
  ...
  const menorTarifa = Math.min(
    ...rotasAtuais.map((r) => r.tarifaComum ?? Infinity).filter(isFinite)
  );

  // Comparativo de tarifas entre empresas
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content={`Horários de ônibus de ${origem} para ${destino}. ${totalEmpresas} empresa${totalEmpresas !== 1 ? "s" : ""} disponível${totalEmpresas !== 1 ? "is" : ""}.`}
        />
      </Head>
      ...
      <div className="min-h-screen bg-background pb-24">
        <div className="mx-auto max-w-3xl px-4 py-6 space-y-5">
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              ...
              <div className="rounded-xl bg-muted/40 px-3 py-2 text-center">
                ...
                <p className="text-lg font-bold text-foreground">
                  {rotasAtuais.reduce((acc, r) => acc + r.horarios.filter(
                    (h) => h.diaDaSemana === diaAtivo && h.sentido === "ida"
                  ).length, 0)}
                </p>
              </div>
          ...
          {/* Comparativo de tarifas (quando há múltiplas empresas) */}
          {temMultiplasEmpresas && (
            <div className="rounded-2xl border bg-card p-5 shadow-sm">
              ...
              <div className="space-y-2">
                {rotasAtuais
                  .filter((r) => r.tarifaComum)
                  .sort((a, b) => (a.tarifaComum ?? 0) - (b.tarifaComum ?? 0))
                  .map((rota) => {
                    const slug = rota.empresa.slug;
                    ...
                    return (
                      <div key={rota.id} className={`flex items-center justify-between rounded-xl px-4 py-3 border ${eMaisBarata ? "border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-900/20" : "bg-muted/20"}`}>
                        <div className="flex items-center gap-2">
                          <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium border ${corBadge}`}>
                            {rota.empresa.nome}
                          </span>
                          {eMaisBarata && (
          ...
          {rotasAtuais.length > 0 ? (
            <div className="space-y-4">
              {rotasAtuais.map((rota) => (
                <CardEmpresa key={rota.id} rota={rota} diaAtivo={diaAtivo} />
              ))}
            </div>
          ) : (
            <div className="rounded-2xl border-2 border-dashed bg-card p-10 text-center">
              ...
              <p className="text-muted-foreground font-medium">
                Nenhuma rota encontrada para {origemAtual} → {destinoAtual}.
              </p>
              <p className="text-sm text-muted-foreground mt-1">
                Tente a direção inversa ou verifique a busca principal.
              </p>
            </div>
          )}
          ...
          <div className="rounded-2xl border bg-card p-5 shadow-sm space-y-3">
            ...
            <div className="space-y-2 text-sm text-muted-foreground">
              ...
              <p className="text-xs border-t pt-2">
                ...
                Horários sujeitos a alteração. Confirme sempre com a empresa antes de viajar.
              </p>
            </div>
          </div>

          {/* Link para rota inversa */}
          {rotasVolta.length > 0 && sentido === "ida" && (
            <Link
              href={`/rota/${slugVolta}`}
              className="flex items-center justify-between rounded-2xl border bg-card p-4 hover:bg-muted/30 transition-colors shadow-sm group"
            >
              <div>
                <p className="text-sm font-semibold text-foreground">
                  Ver página completa da volta
                </p>
